...
int any_zombie_in_range_is_destroyed(
	struct game_state* simulated_state,
	int critical_dist_2) 
{
	if (simulated_state->zombie_count <= 0) {
		return 0;
	}
	std::vector<int> killed_zombie_index(simulated_state->zombie_count);
	zero_array(killed_zombie_index.data(), killed_zombie_index.size());
	...
	if (zombie_killed) {
		simulated_state->zombie_count =
			vacuum(killed_zombie_index.data(), simulated_state->zombie,
...
int zombies_eat_human(struct game_state* simulated_state) {
	...
	std::vector<int>  killed_human_index(simulated_state->human_count);
	zero_array(killed_human_index.data(), killed_human_index.size());
	...
	if (human_killed) {
		simulated_state->human_count =
			vacuum(killed_human_index.data(), simulated_state->human,
